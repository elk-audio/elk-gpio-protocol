...
public:
    ...
    /**
    * @brief Get a tx packet which has been generated by the gpio client. If a
    *        packet exists it will return true and store the address of the new
    *        gpiopacket in packet. Else it will return false and store nullptr
    *       in packet.
    * @param packet pointer where the address of the new gpiopacket will be
     *       stored.
    * @return True if packet exists, false if not
    */
    inline bool get_tx_packet(GpioPacket** packet)
    {
        return _tx_packet_fifo.get_packet(packet);
    }

    /**
     * @brief Get a log msg from the log message fifo. If a log msg exists,
     *        the addres of the new message is stored in msg and true is
     *        returned, else nullptr is stored and false is returned.
